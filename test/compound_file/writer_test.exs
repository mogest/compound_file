defmodule CompoundFile.WriterTest do
  ...
  test "nothing has changed since I last manually tested it" do
    ...
    assert Reader.stream_data(binary, streams |> Enum.at(0)) == long_content
    assert Reader.stream_data(binary, streams |> Enum.at(1)) == "Hello, World!"
  end
  ...
  test "edge case for FAT allocation" do
    ...
    assert Reader.stream_data(binary, stream) == long_content
  end
  ...
  test "mini files only" do
    ...
    assert Reader.stream_data(binary, streams |> Enum.at(0)) == "abc"
    assert Reader.stream_data(binary, streams |> Enum.at(1)) == short_content
    assert Reader.stream_data(binary, streams |> Enum.at(2)) == "hello"
    assert Reader.stream_data(binary, streams |> Enum.at(3)) == String.duplicate("b", 65)
  end
end
